 ...
 */
class WC_Gateway_Alphapaypal_PDT_Handler extends WC_Gateway_Alphapaypal {
	...
	public function check_response() {
		...
		if ( $transaction_result ) {
			WC_Gateway_Alphapaypal::log( 'PDT Transaction Result: ' . wc_print_r( $transaction_result, true ) );
			...
			update_post_meta( $order->get_id(), '_transaction_id', $transaction );
	 		...
	 		if ( 'completed' === $status ) {
				if ( $order->get_total() != $amount ) {
					WC_Gateway_Alphapaypal::log( 'Payment error: Amounts do not match (amt ' . $amount . ')', 'error' );
			...
			WC_Gateway_Alphapaypal::log( 'Received invalid response from PayPal PDT' );
		}
	}
}
