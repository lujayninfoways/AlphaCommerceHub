...
include_once( dirname( __FILE__ ) . '/paypal-response.php' );

/**
 * Handles responses from PayPal IPN.
 */
class WC_Gateway_Alphapaypal_IPN_Handler extends WC_Gateway_Alphapaypal_Response {
	...
	public function valid_response( $posted ) {
		if ( ! empty( $posted['custom'] ) && ( $order = $this->get_paypal_order( $posted['custom'] ) ) ) {
			...
			WC_Gateway_Alphapaypal::log( 'Found order #' . $order->get_id() );
			WC_Gateway_Alphapaypal::log( 'Payment status: ' . $posted['payment_status'] );
	...
	public function validate_ipn() {
		WC_Gateway_Alphapaypal::log( 'Checking IPN response is valid' );
		...
		WC_Gateway_Alphapaypal::log( 'IPN Request: ' . wc_print_r( $params, true ) );
		WC_Gateway_Alphapaypal::log( 'IPN Response: ' . wc_print_r( $response, true ) );

		// Check to see if the request was valid.
		if ( ! is_wp_error( $response ) && $response['response']['code'] >= 200 && $response['response']['code'] < 300 && strstr( $response['body'], 'VERIFIED' ) ) {
			WC_Gateway_Alphapaypal::log( 'Received valid response from PayPal' );
			return true;
		}

		WC_Gateway_Alphapaypal::log( 'Received invalid response from PayPal' );

		if ( is_wp_error( $response ) ) {
			WC_Gateway_Alphapaypal::log( 'Error response: ' . $response->get_error_message() );
		}
	...
	protected function validate_transaction_type( $txn_type ) {
		...
		if ( ! in_array( strtolower( $txn_type ), $accepted_types ) ) {
			WC_Gateway_Alphapaypal::log( 'Aborting, Invalid type:' . $txn_type );
			exit;
		}
	}
	...
	protected function validate_currency( $order, $currency ) {
		if ( $order->get_currency() != $currency ) {
			WC_Gateway_Alphapaypal::log( 'Payment error: Currencies do not match (sent "' . $order->get_currency() . '" | returned "' . $currency . '")' );
	...
	protected function validate_amount( $order, $amount ) {
		if ( number_format( $order->get_total(), 2, '.', '' ) != number_format( $amount, 2, '.', '' ) ) {
			WC_Gateway_Alphapaypal::log( 'Payment error: Amounts do not match (gross ' . $amount . ')' );
	...
	protected function validate_receiver_email( $order, $receiver_email ) {
		if ( strcasecmp( trim( $receiver_email ), trim( $this->receiver_email ) ) != 0 ) {
			WC_Gateway_Alphapaypal::log( "IPN Response is for another account: {$receiver_email}. Your email is {$this->receiver_email}" );
	...
	protected function payment_status_completed( $order, $posted ) {
		if ( $order->has_status( wc_get_is_paid_statuses() ) ) {
			WC_Gateway_Alphapaypal::log( 'Aborting, Order #' . $order->get_id() . ' is already complete.' );
			exit;
		}
